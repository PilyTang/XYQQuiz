...
当前版本为 `0.4.4`，重点降低无题时的题面搜索 CPU 开销，保留中文路径、退出流程、默认显卡优先及独立补充题库，详见 [0.4.4 更新说明](docs/releases/v0.4.4.md)。Windows 11 x64 已验证；Windows 10 1903 及以上 x64 是目标兼容范围，但尚未完成实机验证。教师节基础验证范围见 [v0.4 验收记录](docs/v0.4-validation.md)，名称差异与未收录干扰项修复见 [v0.4.1 修复记录](docs/v0.4.1-feedback-fix.md)。

## 直接使用 Windows 便携版

1. 获取 `XYQQuiz-v0.4.4-win10-win11-x64.zip` 和同名 `.sha256`。
...
.\scripts\build-release.ps1 -Version 0.4.4 -Commit working-tree -AllowDevelopmentCommit
```

正式发布构建必须从干净提交运行，并传入完整 40 位 Git SHA：

```powershell
.\scripts\build-release.ps1 -Version 0.4.4 -Commit (git rev-parse HEAD)
```
