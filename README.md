...
- 自动识别科举和教师节活动，无需手动切换。
- 教师节内置 336 条官方图标和 18 条独立补充记录，共 354 条；所有来源均兼容薄边框和装饰边框，官方更新不会覆盖补充库。
...
当前版本为 `0.5.0`，加入首次硬件自动分档、低配模式及 CPU/GPU 统一框选样式，保留中文路径、退出流程、默认显卡优先及独立补充题库，详见 [0.5.0 更新说明](docs/releases/v0.5.0.md)。Windows 11 x64 已验证；Windows 10 1903 及以上 x64 是目标兼容范围，但尚未完成实机验证。教师节基础验证范围见 [v0.4 验收记录](docs/v0.4-validation.md)，名称差异与未收录干扰项修复见 [v0.4.1 修复记录](docs/v0.4.1-feedback-fix.md)。

## 直接使用 Windows 便携版

1. 获取 `XYQQuiz-v0.5.0-win10-win11-x64.zip` 和同名 `.sha256`。
...
0.5.2 本地验证版增加“开始性能记录 / 停止性能记录”和“导出耗时报告”：正常连续答题后导出逐题及各阶段的中位数、P95。记录不包含截图、题目或答案；退出前需导出。操作与测量边界见 [性能记录说明](docs/performance-recording.md)。
...
.\scripts\build-release.ps1 -Version 0.5.0 -Commit working-tree -AllowDevelopmentCommit
```
...
- Windows 10 1909（18363.657）已收到 `windows_capture.pyd / 0xC0000409` 原生捕获崩溃反馈，0.5.0 尚未修复；目标系统范围不代表所有系统和驱动组合已验收。
